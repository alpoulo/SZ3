...
namespace SZ {
    ...
    public:
        ...
        size_t size_est() {
            ...
            return unpred.size() * sizeof(T) + sizeof(int) * 4; 
        }
        ...
        void save(unsigned char *&c) const {
            ...
            *reinterpret_cast<int *>(c) = this->actual_adapt_ctr;
            c += sizeof(int);
            *reinterpret_cast<int *>(c) = packed_inds.size();
            c += sizeof(int);
            *reinterpret_cast<size_t *>(c) = unpred.size();
            ...
            if (!packed_inds.empty()) {
                HuffmanEncoder<int> encoder = HuffmanEncoder<int>();
                encoder.preprocess_encode(packed_inds, 0);
                encoder.save(c);
                outsize = encoder.encode(packed_inds, c);
        ...
        void load(const unsigned char *&c, size_t &remaining_length) {
            ...
            c += sizeof(int);
            int packed_size = *reinterpret_cast<const int *>(c);
            c += sizeof(int);
            ...
            if(packed_size != 0) {
                ...
                auto packed_inds = decoder.decode(c, packed_size);
                this->packed_inds = packed_inds;
                decoder.postprocess_decode();
                remaining_length -= packed_size * sizeof(int);
            }
